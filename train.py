...
def main_worker(gpu, args):
    ...
    train_loader = data.DataLoader(train_data,
                                   ...
                                   drop_last=True)
    val_loaders = {}
    for valset in args.val_split:
        lmdbpath = os.path.split(args.train_lmdb)[0]
        val_lmdb=os.path.join(lmdbpath,valset+".lmdb" )
        val_data = RefDataset(lmdb_dir=val_lmdb,
                            mask_dir=args.mask_root,
                            dataset=args.dataset,
                            split=valset,
                            mode='val',
